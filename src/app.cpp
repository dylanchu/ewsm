...
void ewsm::Application::register_test_hotkey() noexcept {
    ...
    if(!main_window->RegisterHotKey(MainWindow::ID_hotkey_test, settings.hotkeys.capture_modifiers, settings.hotkeys.capture_key_code)) {
        wxMessageBox("Failed to register hotkey", "Error", wxOK | wxICON_ERROR);
    }
}

void ewsm::Application::unregister_test_hotkey() noexcept {
    main_window->UnregisterHotKey(MainWindow::ID_hotkey_test);
}
